...
class TemporalSynchronizer:
    ...
    def calculate_time_offset(self, 
        ...
        if std1 < 0.02 or std2 < 0.02: # 0~1 사이의 Y좌표. 0.02 미만이면 거의 가만히 있는 것
            self.logger.warning(f"Low movement detected! std1={std1:.4f}, std2={std2:.4f}")
        ...
        # 4. 상호 상관 (Cross-Correlation) - Step 18
        ...
        self.logger.info(f"Sync Result: Lag={lag_frames} frames, Offset={offset_ms:.2f}ms, MaxCorr={max_corr:.3f}")
